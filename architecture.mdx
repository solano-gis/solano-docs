...
SAGE is split across four independent Vercel deployments. Each service has a clear answer to two questions any IT review will ask:
...
<AccordionGroup>
  <Accordion title="sage-web — Chat UI + AI orchestration">
    **What it does:** Chat interface, conversation management, AI tool call orchestration, map screenshots, token budget optimization. Bundles domain knowledge from sage-knowledge via `load_skill`.

    **What it accesses:** Only what sage-gov and sage-gis return via MCP, plus bundled read-only skill files. Cannot query databases or GIS services directly. Authentication, moderation, audit logging, disclaimers, and rate limiting live here.
  ...
  <Accordion title="sage-knowledge — Domain knowledge base">
    **What it does:** Domain expertise across 10 skill areas — property research workflows, zoning interpretation, hazard analysis, budget navigation, and more. Bundled into sage-web at build time and loaded via `load_skill`.

    **What it accesses:** Nothing. Static content repository — no runtime, no API, no external access. Build-time dependency of sage-web only.

    **If it breaks:** AI still functions but loses domain-specific guidance. Tool calls work; interpretation quality degrades.
  </Accordion>

  <Accordion title="sage-gov — Public records MCP server">
    **What it does:** Searches and retrieves public government documents — budget, county code, General Plan, org chart, Board of Supervisors meeting minutes.

    **What it accesses:** Static JSON and SQLite files bundled at deploy time. All data is already public. No external API calls. No write access.
  ...
  <Accordion title="sage-gis — Spatial/GIS MCP server">
    ...
    **What it accesses:** Public ArcGIS FeatureServer and MapServer endpoints — the same services that power the county's public parcel viewer. Read-only.
  ...
  <Accordion title="sage-map — Interactive ArcGIS map">
    **What it does:** Interactive ArcGIS web map embedded as an iframe. Accepts commands (pan, zoom, highlight, toggle layers) and returns screenshots for AI vision.

    **What it accesses:** ArcGIS Online web maps via the Maps SDK. Same data as any ArcGIS web application. No MCP tools. No AI.
...
3. sage-web sends message + screenshot to the AI model
...
Every sage-web interaction is instrumented end-to-end and persisted to Vercel Blob storage. Full visibility into tool chains, timing, token usage, errors, and AI responses.
...
- **Dual output**: `prodLog()` writes to both console.log (Vercel runtime logs) and the collector (Blob persistence)
- **Full verbosity**: Tool args, tool results, AI response — all captured. Internal county app, not public SaaS.
- **Size management**: String values capped at 4KB, base64 data stripped
- **Fire-and-forget persistence**: Blob write failure doesn't break the response
- **No external deps**: Uses `@vercel/blob` and `crypto.randomUUID()` (both built-in)
