...
SAGE's responses are driven by more than tool calls. Before answering a specialized question, the AI loads **domain knowledge** — instructions, gotchas, response patterns, and reference material maintained in a dedicated repository.

This is the difference between "here's the raw data" and "here's what this data means."

## How it works

![Knowledge bundling — sage-knowledge repo bundled into sage-web at build time](/images/knowledge-bundling.png)

At startup, sage-web reads `manifest.json` and injects an `<available_knowledge>` block into the system prompt (~750 tokens). This lists available domains. When a query matches a domain, the AI calls `load_skill()` to get the full instructions before answering.

## Three-tier loading

Knowledge loads progressively:
...
<AccordionGroup>
  ...
  <Accordion title="county-overview — General Solano County context">
    Geography, demographics, economy, cities, landmarks, transportation, and history. Grounding context for general county questions.
