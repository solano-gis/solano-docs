...
description: "How SAGE's accuracy, grounding, and safety are measured"
---

SAGE includes an automated evaluation suite that tests the AI against real county data and scores results across four quality dimensions.
...
SAGE's eval suite produces per-category pass rates against verified ground truth from county records. These are behavioral tests checking the AI's actual responses, not unit tests checking code.
...
The evaluator agent is a separate AI (Gemini Flash) that drives the test — composing prompts, reading responses, and scoring criteria. It interacts with SAGE through a real browser session, seeing the same UI a human would.

<Note>
The evaluator is intentionally a different model than the system under test. This prevents the AI from "recognizing" its own patterns.
</Note>
...
The evaluator checks exact matches — "approximately 5.28 acres" passes, but "5.3 acres" would not.
...
- **Trend tracking** — compare `passRate` across runs
...
The eval suite provides quantitative backing for the [Risk & Governance](/governance) page:
