---
title: "sage-web"
description: "Chat UI and AI orchestration"
---
...
Alongside tool groups, sage-web bundles **domain knowledge** from [sage-knowledge](/knowledge) — expertise across 10 skill areas. The AI discovers available skills via an `<available_knowledge>` XML block in the system prompt (~750 tokens), then loads specific skills on demand via `load_skill()`.

Tool groups provide **tool schemas** (what tools are available), while skills provide **domain knowledge** (how to interpret results, what workflows to follow, what gotchas exist). See [Knowledge Base](/knowledge).
...
- **Scope:** Solano County focus (7 cities + unincorporated), off-topic redirects
- **Jurisdiction routing:** Automatic detection that mailing address ≠ legal jurisdiction
- **Response scaling:** Quick answers for simple lookups, detailed answers for policy research
...
`update_map` waits up to 8 seconds for the iframe's `command-ack` before calling `addToolOutput`. The tool output includes an `ackStatus` field (`confirmed`, `unconfirmed`, or `failed`) so the AI knows whether it can trust the map state. See [Map Protocol](/integration/map-protocol) for details.
...
This gives the AI metadata to run follow-up queries on the same selection using `query_map_features` — e.g., `stats` mode for value distributions or `sample` mode for top-N lists — without auto-sending attribute data that could fill the context window. The AI decides whether a follow-up is worth the tokens based on the user's question.
