...
SAGE provides oblique (angled) aerial imagery for any location in Solano County. Unlike standard nadir (top-down) aerials, oblique views show building facades, roof details, and terrain from an angle — useful for property assessment, damage inspection, and understanding 3D context.
...
The AI uses these naturally — "zoom in on the roof" triggers zoom 20 with nadir direction, "show me the neighborhood" triggers zoom 16.

## Pipeline architecture

```
                Sanborn FindFootprints API
                        │
                        ▼
                ┌───────────────┐
                │  sage-gis     │
                │  MCP tool     │─── metadata (directions, URLs, footprints)
                └───────┬───────┘
                        │ self-call via production URL
                        ▼
                ┌───────────────┐
                │  /api/        │
                │  oblique-     │─── fetches COG tiles via range requests
                │  preview      │    from services.sanborn.com
                └───────┬───────┘
                        │
                        ▼
                  JPEG crop image
